        ...
        <CardContent className="flex-1 space-y-4">
            ...
            <ul className="space-y-1">
              {includedScripts.map(script => (
                <li key={script.id} className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-muted-foreground">({formatPrice(script.monthlyPriceCents)}/mois)</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="pt-4 border-t space-y-2">
            <div className="flex items-baseline justify-between">
              ...
              <span className="text-sm line-through text-muted-foreground">{formatPrice(annualPrice)}/an</span>
            </div>
            <div className="flex items-baseline justify-between">
              <span className="font-medium">Votre prix:</span>
              <span className="text-2xl font-bold text-primary">{formatPrice(discountedPrice)}/an</span>
            </div>
            <div className="flex items-baseline justify-between text-sm">
              <span className="text-muted-foreground">Soit:</span>
              <span className="text-muted-foreground">{formatPrice(monthlyEquivalent)}/mois</span>
            </div>
            <div className="flex items-center justify-end gap-1 text-green-600">
              <span className="text-sm font-medium">Economie: {formatPrice(savings)}</span>
            </div>
          </div>
        </CardContent>
